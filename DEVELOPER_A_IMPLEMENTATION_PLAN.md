...
| TB2 生产 Evidence Graph / Case Compiler | COMPLETE | 已完成：TB2a（Golden 系统案件种子 + getPublic/getSource）与 TB2b（用户案件完整编译器：版本化候选 schema、纯函数编译器定 4+1/答案/unlock rule/rubric、完整校验与原子落库；真实模型端到端通过）。见 [TB2a handoff](./docs/handoffs/2026-09-05-tb2a-golden-seed-public-queries.md) 与 [TB2b handoff](./docs/handoffs/2026-09-05-tb2b-user-case-compiler.md)。 |
...
| P1-3 第二案件 | IN PROGRESS | 实现与静态验证完成：第二真实来源经真实模型 durable 编译成功（claim-extraction@2 / case-compilation@2，新增解锁覆盖约束与抽取硬化），案件不变量 + Span 逐字回溯 + 不含第一案工件 + 用户案件不进目录全部断言通过。2026-09-06 供应商按用户决定切换为智谱 `glm-4.7-flash`（本地+生产八项 AI_*、smoke 通过）；原 DeepSeek 余额阻塞解除，但免费档存在时段性过载（错误 1305、非流式请求约 900s 被供应商切断），`COMPILE_LEASE_MS` 已按新供应商重校准 15→30 分钟，最终全链闭环以探测门控重试在健康窗口收口。见 [P1-3 handoff](./docs/handoffs/2026-09-05-p13-g1-second-case.md) 与 [AI 切换 handoff](./docs/handoffs/2026-09-06-ai-provider-switch-glm.md)。 |
