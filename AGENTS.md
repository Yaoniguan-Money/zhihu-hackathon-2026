...
- 只有三种已批准的恢复：角色候选（忠实或篡改）语义失败后带反馈语义重写，共 10 次候选（首试 + 9 次重写；2026-09-11 经用户批准纳入篡改角色并由"两次"上调至 10 次；非法引用与引入新事实仍立即终止）；ASR/TTS 失败时展示错误并保留人工文字路径或已批准文字；模型调用网络类瞬时错误（连接断开 / 5xx / 429 / 408）最多传输层自动重试 2 次（2026-09-11 用户批准；schema 解析、协议、4xx 与配置错误不重试）。除此之外不得增加兜底。
