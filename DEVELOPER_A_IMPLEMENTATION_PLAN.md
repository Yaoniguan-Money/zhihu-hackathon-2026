...
| P1-2 本地 Voice | IN PROGRESS | P1-2a 基建已完成（2026-09-05）：本地 Worker（仅绑 127.0.0.1，SenseVoice+FSMN-VAD+Kokoro，模型/音色 143 文件 sha256 清单固定、离线缓存）、两个同源 Voice Route、Approved Speech Envelope 落库、ASR/TTS 幂等持久化（Convex storage）；TTS→ASR 往返与全链 e2e 通过。见 [P1-2a handoff](./docs/handoffs/2026-09-05-p12-2a-local-voice-infra.md)。收口剩余：五音色经 A/B 真人试听锁定（`voice-pack.json`，用户动作）与 REL1 验收。 |
