...
- 浏览器内真实模型**全链试玩 demo**（开场→审讯→证据→指控→Reveal）未走通：GLM 免费档对长结构化回合间歇性失败（详见"已知风险"）。已验证到的最远链路：建 session → `game.start` → `opening_statements` phase → **第一条真实 AI 开场陈述全链成功**（生成→Validator→发布→事件流→快照→打字机 UI，沈青梧完整陈述带情绪/立场徽章上屏）→ 后续回合供应商故障 → 显式终局（四次开庭尝试，失败呈现全部按契约）。此为外部供应商问题，非前端缺陷；UI 代码路径均为真接口、无任何 mock。
