...
- CLI 0.5.0 的能力清单（`capabilities`，2026-09-04 核实）不含“按 URL 读取文章完整正文”的命令；官方开放平台 HTTP API 亦无文章内容端点。Golden Case 需要完整正文时，由用户提供或对用户提供的 URL 做渲染抓取，并在来源元数据中记录获取方式与哈希。
