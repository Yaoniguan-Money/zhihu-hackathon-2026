# ART-CHR-a-lan：Alan Blender 角色资产
...
- 当前工程：`art/blender/characters/a-lan/v007_rigged.blend`；GLB：`art/blender/exports/a-lan.glb`。
- 实测 70132 triangles、92 个独立网格、28 骨骼、9 段动作。
- 五向渲染及六种动作极值渲染已解码核验。视觉观察：五向与参考同角色：红棕蓬乱短发+双翘发束轮廓、珊瑚红连帽衫（芥黄抽绳）、炭灰工装裤、DEV 工牌、左臂平板、双肩包、红白运动鞋均落实；头身比与沈青梧/纪云汀一致，无穿模；动作极值六项均有效。
...
- `art/blender/characters/a-lan/` — 阶段工程、指标与检查记录。
- `art/blender/exports/a-lan.*` — GLB 与实际结构/Three.js 检查结果。
- `art/blender/renders/a-lan/` — 四向 Blockout、五向成品、六个动作极值。
...
- `python art/blender/scripts/inspect_glb.py art/blender/exports/a-lan.glb` — 通过：GLB 容器、内嵌资源、UV、骨骼、动画/morph 通道、三角面预算。
- `node art/blender/scripts/verify_three.mjs art/blender/exports/a-lan.glb` — 通过：实际 GLTFLoader 解析、AnimationMixer 播放、有限数值、归一化权重、口型/眨眼与头部动作。
...
5. `art/blender/characters/a-lan/review.json`
